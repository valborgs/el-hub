...
3. **제외 필터링**:
   - 사용자 정의 Glob 패턴(예: `*.tmp`, `node_modules/`)을 등록하여 불필요한 백업을 차단합니다.
...
└── app/
    ...
    ├── backup_engine.py # 파일 복사, 제외 조건 적용, 초기 동기화 엔진
    ├── watcher.py       # watchdog 래퍼를 활용한 실시간 변경 감시 서비스
    ├── gdrive.py        # Google Drive API 연동 및 OAuth 인증 흐름 제어
...
- [ ] **작업 범위 옵션 다각화**:
  - 기존의 제외(Exclude) 방식 외에 특정 폴더/파일만 선택하여 백업하는 포함(Include) 옵션 분리.
  - 제외/포함 토글 버튼 및 전용 인터페이스 섹션 제공.
- [ ] **GUI 레이아웃 고도화**:

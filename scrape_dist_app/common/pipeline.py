...
실행 절차
    ...
    3단계  diff_core.run_diff_highlight()
           N열 단일 항목 셀에 before/after 비교 Rich Text diff 강조 적용

    4단계  design_core.run_design()
           _자동분류 파일에 테두리·폰트·정렬·행 높이 서식 일괄 적용
           (rich_text=True로 로드해 CellRichText를 보존하면서 스타일 재적용)

공개 함수
    run_pipeline(excel_file, gsheet_index, start_box, end_box, log_callback)
        → 네 단계를 순서대로 실행하고 최종 출력 파일 경로를 반환합니다.
...
from core.diff_core   import run_diff_highlight, patch_xml_space_preserve  # noqa: E402
...
def run_pipeline(
    ...
    Args:
        ...
        run_diff:     True이면 3단계(diff 강조)·5단계(XML 후처리) 실행
    ...
    # ── 3단계: Rich Text diff 강조 ────────────────────────────────────────
    if run_diff:
        log_callback("info", "[ 3단계 ] 수정내역 Rich Text diff 강조")
        run_diff_highlight(
            excel_file   = dst_path,
            log_callback = log_callback,
        )
    else:
        log_callback("info", "[ 3단계 ] 수정내역 Rich Text diff 강조 → 스킵")

    # ── 4단계: 서식 일괄 적용 ─────────────────────────────────────────────
    log_callback("info", "[ 4단계 ] 서식 일괄 적용")
    run_design(
        excel_file   = dst_path,
        log_callback = log_callback,
    )

    # ── 5단계: XML 후처리 (openpyxl 3.1.5 버그 우회) ──────────────────────
    if run_diff:
        log_callback("info", "[ 5단계 ] worksheet XML 후처리")
        patch_xml_space_preserve(
            excel_file   = dst_path,
            log_callback = log_callback,
        )
    else:
        log_callback("info", "[ 5단계 ] worksheet XML 후처리 → 스킵")

...
SHEET_TARGET_RATIO = {1: 0.80, 2: 0.40, 3: 0.20, 4: 0.10}  # 시트별 목표 분배 비율(행 기준)
```

#### 시트별 분배 비율(쿼터 기반)
- **기준 단위**: "내용(L/M/N)이 채워진 제어번호 행 수"
- **배치 원칙**: 한 오류 항목 = 한 시트(partition). 같은 항목을 중복 배치하지 않음
- **1차 점검**: 항목이 있으면 첫 항목을 우선 배정하고, 다른 시트 페이스를 넘는 잉여 항목도 모두 흡수하는 **catch-all(상한 없음)**. 따라서 1차 행 수 ≈ *항목이 있는 행 수*이며, 목표 80%는 "약 20%의 행이 수정내역이 없는 경우"에 자연히 맞음(데이터가 더 조밀하면 80%를 초과해 최대 100%까지 올라감)
- **2·3·4차**: `SHEET_TARGET_RATIO`를 **행 위치 기준 페이스(pacing)**로 사용. 현재 행 위치 `pos`까지의 목표 누적치 `SHEET_TARGET_RATIO[s] × pos`보다 뒤처진 시트부터 한 항목씩 채움. 이렇게 하면 **처음부터 마지막 행까지 일정 간격으로 고르게** 분포하고, 데이터에 항목이 충분하면 목표(40/20/10%)에 도달
  - ⚠️ 전체 누적 상한(`filled ≥ ratio × total_rows`)만 쓰면 **앞쪽 행이 쿼터를 모두 소진**해 뒤쪽 행이 비는 front-loading이 발생하므로, 반드시 행 위치 기준 페이스를 사용
- **3·4차 제약 유지**: `ALLOWED_TYPES_LATE`(오탈자/띄어쓰기) 항목만 받음. 해당 유형이 부족하면 목표보다 적게 채워짐
- **잉여 항목**: 2·3·4차 페이스를 넘는 항목은 버리지 않고 모두 1차로 흡수(데이터 손실 없음)
...
7. **시트별 분배(행 위치 기준 페이스)** — `SHEET_TARGET_RATIO` 비율을 행 수 기준으로 맞추되, 마지막 행까지 고르게 분포
   - 사전에 `sheet_filled[s] = 0` 준비(루프 진입 전 1회)
   - 현재 행 위치 `pos = idx + 1` 사용
   - **① 1차 점검**: 항목이 있으면 첫 항목을 우선 배정(주 점검 시트). 상한 없음
   - **② 2·3·4차**: 남은 항목을 대상으로, 행 위치 기준 목표 누적치 `paced_target = SHEET_TARGET_RATIO[s] × pos`보다 **가장 많이 뒤처진(`paced_target − sheet_filled[s]`가 큰) 시트부터** 한 항목씩 배정. 각 시트는 한 행에서 최대 1회. `sheet_filled[s] >= paced_target`(페이스 충족) 시트는 이후 행을 위해 양보
   - **③ 3·4차 제약**: `ALLOWED_TYPES_LATE`(오탈자/띄어쓰기) 항목만 받음. 후보 풀에 해당 유형이 없으면 그 시트는 건너뜀
   - **④ 잉여 항목**: 위 단계에서 배치되지 못하고 남은 항목은 모두 1차 점검으로 흡수(버리지 않음)
   - 결과: 2·3·4차는 목표 비율(예: 40/20/10%)에 수렴하면서 **처음~마지막 행까지 고르게** 분포, 1차는 "항목이 있는 행 수"(≈ 80%~)가 됨
   - ⚠️ 과거 전체 누적 상한 방식은 앞쪽 행이 쿼터를 모두 소진해 뒤쪽 행이 비는 front-loading 버그가 있었음 → 행 위치 기준 페이스로 해결
